...
const GENERATOR_KAR_BOOT = 'generator-kar-boot';
...
/**
 * Add a new element to the build.gradle file.
 *
 * @param {string} gradleFileNameApplyFromStatement - The 'apply from: name_0f_gradle_file' statement
 */
Generator.prototype.addElementToTheGradleFile = function (gradleFileNameApplyFromStatement) {
    try {


        fs.appendFile('build.gradle', '\n' + gradleFileNameApplyFromStatement, function (err) {
          if (err) throw err;
          console.log('The "data to append" was appended to file!');
        });
    } catch (e) {
        this.log(chalk.yellow('\nUnable to find ') + 'build.gradle' + chalk.yellow(' or missing required jhipster-needle. Reference to not added to the build script.\n'));
    }
};

/**
 * Add a new element to the gradle.properties file.
 *
 * @param {string} propertyVersionInformation - The 'property=value' statement
 */
Generator.prototype.addElementToTheGradlePropertiesFile = function (propertyVersionInformation) {
    try {



        fs.appendFile('gradle.properties', '\n' + propertyVersionInformation, function (err) {
          if (err) throw err;
          console.log('The "data to append" was appended to file!');
        });
    } catch (e) {
        this.log(chalk.yellow('\nUnable to find ') + 'gradle.properties file' + chalk.yellow(' or missing required jhipster-needle. Reference to not added to the build script.\n'));
    }
};

/**
 * Appends fileA to fileB
 *
 * @param {string} fileA - the content of the file to be copied (appended) into another file
 * @param {string} fileB - file to be modified
 */
Generator.prototype.appendFileAToFileB = function (fileA, fileB) {
  try {
    fs.readFile(fileA, function (err, data) {
      if (err) throw err;
      fs.appendFile (fileB, '\n' + data, function(err) {
        if (err) throw err;
        console.log('Appended file ' + fileA + " to file " + fileB);
      });
    });
  } catch (e) {
    this.log(chalk.yellow('\nUnable to append ') + fileA + ' to ' + fileB + chalk.yellow(' or missing required jhipster-needle. Reference to not added to the build script.\n'));
  }
};
...
Generator.prototype.registerModule = function (npmPackageName, hookFor, hookType, callbackSubGenerator, description) {
    try {
        var modules;
        var error, duplicate;
        var moduleName = _.startCase(npmPackageName.replace(GENERATOR_KAR_BOOT + '-', ''));
 ...
 */
Generator.prototype.printLogo = function (appendedText) {

    clear();
    this.log(chalk.red(figlet.textSync('KAR-BOOT: ' + appendedText, { horizontalLayout: 'full' })));
    this.log(chalk.white.bold('                            https://rhys-parry@bitbucket.org/rhys-parry/generator-kar-boot.git\n'));
    if (!this.skipChecks) this.checkForNewVersion();
    this.log(chalk.white('Welcome to the KAR Spring-Boot Generator ') + chalk.yellow('v' + packagejs.version));
 ...
 * Prints a JHipster logo.
 */
Generator.prototype.printKata = function (appendedText) {

    clear();
    this.log(chalk.red(figlet.textSync(appendedText, { horizontalLayout: 'full' })));
...
Generator.prototype.checkForNewVersion = function () {
    try {
        shelljs.exec('npm show ' + GENERATOR_KAR_BOOT + ' version', {silent:true}, function (code, stdout, stderr) {
            if (!stderr && semver.lt(packagejs.version, stdout)) {
                this.log(
                    ...
                    chalk.yellow('  Kar-boot update available: ') + chalk.green.bold(stdout.replace('\n','')) + chalk.gray(' (current: ' + packagejs.version + ')') + '\n' +
                    chalk.yellow('  Run ' + chalk.magenta('npm install -g ' + GENERATOR_KAR_BOOT ) + ' to update.\n') +
                    chalk.yellow(' ______________________________________________________________________________\n')
                );
            }
        }.bind(this));
